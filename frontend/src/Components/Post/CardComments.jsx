       ...
       <div className="comments-container">
           ...
           {(commentsData[0]) && commentsData.map((comment)=>{
               return(
                        ...
                        usersData.map((user)=>{
                            if(user.id === comment.user_id) return <div className="pseudo" key={user.id}>{user.name} {user.firstname}</div>;
                               ...
                               <p className="comment-text">{comment.message}</p>
                                {((userData.admin === 1 || (comment.user_id === userData.id)) && commentUpdated === true) && 
                   ...
                   {((userData.admin === 1 || (comment.user_id === userData.id)) && edit === false) && <FiMoreVertical onClick={(e) => storeCommentIdAndSetEdit(comment.id)}/>}
                   {((userData.admin === 1 || (comment.user_id === userData.id)) && edit === true) && 
